...
spdiagram=0; %spot diagram (dots on sensor plane)
spImage=0;%spot image: For accurate spot shape simulation choose numrays>30
...
spdiagram=1; %spot diagram (dots on sensor plane)
spImage=1;%spot image: For accurate spot shape simulation choose numrays>30
...
Wv=[486 656]; % in nm, can specify multiple wavelengths by Wv=[630 800], for chromatic sim
...
indexInputMode=checkIndexHomogeneity(E); %checks homogeneity, all indices must be either numeric or material
if length(Wv)>1, ChromaticMode=1; else; ChromaticMode=0;end
if ChromaticMode
    if ~strcmpi(indexInputMode,'string')
        error('Cannot use direct index input in Chromatic simulations, use materials instead')
    end
end
...
for wi=1:length(Wv)
    raycmpM=[];cntB=1;
    E=resolveIndicesOneLambda(E0,1e-3*Wv(wi));
    RayM=ray;
        while length(RayM>1)
            ...
            %this is not effecient, fix later
            RayM=RayM(2:end,:);
        end
    raycmpM_Wv{wi}=raycmpM;
end

[rgb, lineStyle, labels] = wavelengthColors(Wv);
...
if rtplot
    ...
    if ~ChromaticMode
        ...
        for wi=1:length(Wv)
            raycmpM=raycmpM_Wv{wi};
            [num5,~]=size(raycmpM);
            for i=1:num5
                plot(raycmpM(i,[1 14]),raycmpM(i,[3 16]),'linewidth',5*raycmpM(i,7),'color',rgb(wi,:));
            end
        end
    end
    %debugging
...
for wi=1:length(Wv)
    ...
    raySens_Wv{wi}=raySens;
end


if strcmpi(Task,'find focus')
         'Focus Calculated on last Wavelength'
...
%}
if spdiagram
    figure (113); hold on; axis equal;title('Spot Diagram')

    for wi=1:length(Wv)
        raySens=raySens_Wv{wi};
        scatter(raySens(:,15),raySens(:,16),'MarkerFaceColor',rgb(wi,:));
    end
    if strcmpi(Sensor.Type,'circular')
...
if spImage %spot plot
      if ChromaticMode
           'Warning: Spot Image are not implemented for Chromatic Simulations, for now';
      elseif
...
if plot3dr
    if ~ChromaticMode
        ...
        [rgb, lineStyle, labels] = wavelengthColors(Wv);
        Display3dLayout2(raycmpM_Wv{wi},E,1,rgb(1,:),lineStyle{1});
        for wi=2:length(Wv)
              Display3dLayout2(raycmpM_Wv{wi},E,0,rgb(wi,:),lineStyle{wi});
        end
    end
