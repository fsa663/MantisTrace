function [y2]=interpolateRays(y2,numrays,res)
    ...
    %7:power, 8:pathlength,9:reserved,10:reflection order ,11=i, 12=j, 13 =id of originator element
    %14,15,16 terminal position
    Q=[];
    rayordmax=max(y2(:,10));
    ...
    for t=1:rayordmax
        for i=1:numrays
            for j=1:numrays
                ...
                if length(idx)==0,
                        Q1(i,j,:)=zeros(1,1,16);Q1(i,j,10)=t;
                else
                        Q1(i,j,:)=y2(idx,:);
                end
            end
    ...
    %Force the power sum to be the same after interpolation
    y22(:,7)=(sum(y2(:,7))/sum(y22(:,7))) * y22(:,7);
